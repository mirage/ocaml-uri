...
let uri_encodes =
  [
    ( "https://user:pass@foo.com:123/wh/at/ever?foo=1&bar=5#5",
      Uri.make ~scheme:"https" ~userinfo:"user:pass" ~host:"foo.com" ~port:123
        ~path:"/wh/at/ever"
        ~query:[ ("foo", [ "1" ]); ("bar", [ "5" ]) ]
        ~fragment:"5" () );
    ("http://foo.com", Uri.make ~scheme:"http" ~host:"foo.com" ());
    ("http://foo-bar.com", Uri.make ~scheme:"http" ~host:"foo-bar.com" ());
    ( "http://foo-bar.com:8080/h1",
      Uri.make ~scheme:"http" ~host:"foo-bar.com" ~port:8080 ~path:"/h1" () );
    ...
    ( "http://user:pass@[2001:41d1:fe67:500:227:c6ff:fe5a:efa0]:6789/wh/at/ever?foo=1&bar=5#5",
      Uri.make ~scheme:"http" ~userinfo:"user:pass"
        ~host:"[2001:41d1:fe67:500:227:c6ff:fe5a:efa0]" ~port:6789
        ~path:"/wh/at/ever"
        ~query:[ ("foo", [ "1" ]); ("bar", [ "5" ]) ]
        ~fragment:"5" () );
    ...
    ( "http://user:pass@%5B2001%3A41d1%3Afe67%3A500%3A227%3Ac6ff%3Afe5a%3Aefa0%25wlan0%5D:6789/wh/at/ever?foo=1&bar=5#5",
      Uri.make ~scheme:"http" ~userinfo:"user:pass"
        ~host:"[2001:41d1:fe67:500:227:c6ff:fe5a:efa0%wlan0]" ~port:6789
        ~path:"/wh/at/ever"
        ~query:[ ("foo", [ "1" ]); ("bar", [ "5" ]) ]
        ~fragment:"5" () );
...
let test_pct_large =
  ...
  @ map_pcts_tests "large" "decode"
      (fun a b -> (Uri.pct_decode b, a))
      pcts_large
...
let test_uri_encode =
  List.map
    (fun (uri_str, uri) ->
      ...
      let test () =
        assert_equal ~printer:(fun x -> x) uri_str (Uri.to_string uri)
      in
      name >:: test)
    uri_encodes
    ...
    (fun (uri_str, uri) ->
      ...
      let test () =
        assert_equal
          ~printer:(fun x -> x)
          uri_str
          Uri.(to_string (of_string (Uri.to_string uri)))
      in
      name >:: test)
    uri_encodes
...
let test_query_encode =
  List.map
    (fun (qs, res) ->
      let test () =
        assert_equal ~printer:(fun l -> l) res (Uri.encoded_of_query qs)
      in
      res >:: test)
    uri_query_make
    ...
    (fun (rel, abs) ->
      let test () =
        assert_equal
          ~printer:(fun l -> l)
          abs
          (Uri.to_string (Uri.resolve "http" base (Uri.of_string rel)))
      in
      rel >:: test)
    uri_rel_res

let file_uri_rel_res =
  [
    (* http://tools.ietf.org/html/rfc1738#section-3.10 *)
    ...
    (fun (rel, abs) ->
      let test () =
        assert_equal
          ~printer:(fun l -> l)
          abs
          (Uri.to_string (Uri.resolve "file" Uri.empty (Uri.of_string rel)))
      in
      rel >:: test)
    file_uri_rel_res
    ...
    (fun (rel, res) ->
      let test () =
        assert_equal
          ~printer:(fun l -> l)
          res
          Uri.(to_string (resolve "" empty (of_string rel)))
      in
      rel >:: test)
    rel_empty_path_res
    ...
    (fun (rel, base, res) ->
      ...
      let test () =
        assert_equal
          ~printer:(fun l -> l)
          res
          (Uri.to_string (Uri.resolve "" base rel))
      in
      res >:: test)
    uri_rel_rel_res
           ...
           (fun (uri, res) ->
             ...
             let test () =
               assert_equal
                 ~printer:(fun l -> l)
                 res
                 (Uri.to_string (Uri.resolve "" base uri))
             in
             res >:: test)
           tests)
...
let generic_uri_norm =
  [
    ...
    ( "//colon%3Auser:colon%3Apassword@example.net/",
      "//colon%3Auser:colon%3Apassword@example.net/" );
    ("//colon%3Auser@example.net/", "//colon%3Auser@example.net/");
    ("foo+bar%3a", "./foo+bar:");
    ( "http://[2001:DB8:1234:5678:90ab:cdef::0123]/%68%65%6c%6c%6f",
      "http://[2001:db8:1234:5678:90ab:cdef::0123]/hello" );
    ...
    (fun (o, n) ->
      let test () =
        assert_equal
          ~printer:(fun l -> l)
          n
          (Uri.to_string (Uri.resolve "http" Uri.empty (Uri.of_string o)))
      in
      o >:: test)
    generic_uri_norm
...
let test_rel_id =
  List.map
    (fun id ->
      let test () =
        assert_equal ~printer:(fun l -> l) id (Uri.to_string (Uri.of_string id))
      in
      id >:: test)
    rel_id
...
let test_tcp_port_of_uri =
  ...
  List.map
    (fun (uri, pn) ->
      let test () =
        assert_equal ~printer:string_of_int_option (Some pn)
...
let test_with_change =
  [
    ...
    ( "test_with_host" >:: fun () ->
      ...
      let uri_exp = "" in
      let msg =
        sprintf "host removal with None (%s <> %s)" uri_exp
          (Uri.to_string uri_none)
      in
      assert_equal ~msg (Uri.of_string uri_exp) uri_none;
      let uri_exp = "//" in
      let uri_some_empty = Uri.with_host uri (Some "") in
      let msg =
        sprintf "host removal with empty (%s <> %s)" uri_exp
          (Uri.to_string uri_some_empty)
      in
    ...
    ( "test_with_port" >:: fun () ->
      ...
      let uri_exp = "//:80" in
      let msg =
        sprintf "add port to empty (%s <> %s)" uri_exp (Uri.to_string uri_port)
      in
    ...
    ( "test_with_path" >:: fun () ->
      ...
      let msg =
        sprintf "empty host percent / path (%s <> %s)" (Uri.to_string uri_pct)
          "a%2F"
      in
      ...
      let msg =
        sprintf "some host empty path (%s <> %s)" (Uri.to_string uri)
          (Uri.to_string uri_empty)
      in
      ...
      let msg =
        sprintf "path relative host (%S <> %S)" uri_exp_str uri_some_str
      in
    ...
    ( "test_with_query" >:: fun () ->
      ...
      let test_with_query prefix =
        ...
        let msg =
          sprintf "'%s' quest (%S <> %S)" prefix uri_exp_str uri_quest_str
        in
...
let with_uri =
  let base =
    Uri.of_string "scheme://user:pass@host:0/path?query=arg#fragment"
  in
  [
    (Uri.with_uri base, Uri.to_string base);
    ( Uri.with_uri ~scheme:None base,
      "//user:pass@host:0/path?query=arg#fragment" );
    ( Uri.with_uri ~scheme:(Some "new") base,
      "new://user:pass@host:0/path?query=arg#fragment" );
    (Uri.with_uri ~userinfo:None base, "scheme://host:0/path?query=arg#fragment");
    ( Uri.with_uri ~userinfo:(Some "new") base,
      "scheme://new@host:0/path?query=arg#fragment" );
    ( Uri.with_uri ~host:None base,
      "scheme://user:pass@:0/path?query=arg#fragment" );
    ( Uri.with_uri ~host:(Some "new") base,
      "scheme://user:pass@new:0/path?query=arg#fragment" );
    ( Uri.with_uri ~port:None base,
      "scheme://user:pass@host/path?query=arg#fragment" );
    ( Uri.with_uri ~port:(Some 1) base,
      "scheme://user:pass@host:1/path?query=arg#fragment" );
    ( Uri.with_uri ~path:None base,
      "scheme://user:pass@host:0?query=arg#fragment" );
    ( Uri.with_uri ~path:(Some "new") base,
      "scheme://user:pass@host:0/new?query=arg#fragment" );
    (Uri.with_uri ~query:None base, "scheme://user:pass@host:0/path#fragment");
    ( Uri.with_uri ~query:(Some [ ("new", [ "a" ]) ]) base,
      "scheme://user:pass@host:0/path?new=a#fragment" );
    ( Uri.with_uri ~fragment:None base,
      "scheme://user:pass@host:0/path?query=arg" );
    ( Uri.with_uri ~fragment:(Some "new") base,
      "scheme://user:pass@host:0/path?query=arg#new" );
...
let test_with_uri =
  List.map
    (fun (input, output) ->
      input >:: fun () -> assert_equal ~printer:(fun l -> l) output input)
  ...
  List.map
    (fun (input, expected) ->
      ...
      let test () =
        ...
        | Ok parsed -> assert_equal ~printer:(fun x -> x) expected parsed
...
let test_make_path_rel_identity =
  ...
  assert_equal ~printer:Uri.to_string ~cmp:Uri.equal u u'
...
let eval_rfc9110_uris tests ~f =
  List.map
    (fun input ->
      ...
      let test () =
        Uri.of_string input |> Uri.Absolute_http.of_uri |> f ~input
      in
      name >:: test)
    tests
...
let test_parseable_rfc9110_uris =
  let eval ~input = function
    | Ok http_uri -> assert_equal (Uri.Absolute_http.to_string http_uri) input
...
let test_unparseable_rfc9110_uris =
  let eval ~input = function
    | Ok _ ->
        assert_failure
          (sprintf "unexpected conversion of non-rfc9110 uri: %s" input)
    | Error _ -> ()
  in
...
let _ =
  let suite =
    "URI"
    >::: test_pct_small @ test_pct_large @ test_uri_encode @ test_uri_decode
         @ test_query_decode @ test_query_encode @ test_rel_res
         @ test_file_rel_res @ test_rel_rel_res @ test_userinfo_res
         @ test_rel_empty_path_res @ test_generic_uri_norm @ test_rel_id
         @ test_tcp_port_of_uri @ query_key_add_remove @ test_with_change
         @ test_canonicalize @ test_with_uri @ test_ipv6_parsing
         @ test_parseable_rfc9110_uris @ test_unparseable_rfc9110_uris
